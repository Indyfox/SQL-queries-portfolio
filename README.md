...
| 3 | Оконные функции над агрегатами | Максимально лаконично: все расчёты в одном CTE. Элегантное использование оконных функций после GROUP BY. | Может быть сложно для понимания новичками. |
...
| 2 | Оконные функции в одном CTE | Очень компактно, все нужные поля вычисляются на лету. | При больших объёмах данных оконные функции могут быть тяжелее. Одно обращение к таблице. |
| 3 | CTE с включением first_order_date в нумерацию | Самый сбалансированный: все нужные поля в одном CTE, хорошая читаемость. Последовательная фильтрация. |
...
| # | Подход | Плюсы | Минусы |
|---|--------|-------|--------|
| 1 | Раздельные CTE для продаж и возвратов | Чёткая структура, легко контролировать фильтры. |
| 2 | Один запрос с LEFT JOIN и оконной функцией | Компактность, все данные собираются в одном месте. Правильно используется MIN для tie-break. | |
...
#### Решение 3.2 (один CTE с LEFT JOIN и оконной функцией)
```sql
WITH
    ranked_items_by_dept AS
    (
...
| 2 | Компактный запрос с двумя CTE | Лаконичнее, логика понятна. Быстрое получение общей статистики региона. |
...
| # | Подход | Плюсы | Минусы |
|---|--------|-------|--------|
| 1 | Одно CTE с LEFT JOIN и CASE | Все метрики считаются в одном месте, просто и наглядно. Корректно обрабатываются клиенты без заказов благодаря LEFT JOIN. | ... |
...
| # | Подход | Плюсы | Минусы |
|---|--------|-------|--------|
| 1 | Два CTE (sales, avg_sales) | Логичное разделение: сначала недельные продажи, потом средние. Хорошо читается. | ... |
| 2 | Оконные функции в одном CTE | Более компактно, все расчёты в одном месте. Умелое использование оконной функции AVG для получения средней по партиции. | ... |
...
| # | Подход | Плюсы | Минусы |
|---|--------|-------|--------|
| 1 | CTE с LAG | Чётко выделяются обращения нужных клиентов. Использование LAG для сравнения дат. Условие на сегмент проверяется через JOIN в CTE. | ... |
...
WITH 
    ...
    SELECT
        sr.customer_id,
        c.name
        sr.request_date AS cur_date,
        LAG(sr.request_date) OVER(PARTITION BY sr.customer_id ORDER BY sr.request_date) AS prev_date
    FROM
        support_requests sr
    JOIN
        customers c
    ...
    WHERE
        sr.request_date IS NOT NULL
        c.segment = 'retail'
    )
SELECT
    name,
    COUNT(*) AS total_requests,
    SUM(CASE WHEN cur_date BETWEEN prev_date AND prev_date + INTERVAL 9 DAY
        THEN 1 ELSE 0 END) AS short_gaps
FROM
    requests 
GROUP BY
    customer_id,
...
| # | Подход | Плюсы | Минусы |
|---|--------|-------|--------|
| 1 | Оконная функция | Лаконично, сразу виден ранг. Использование `IFNULL` для клиентов без заказов. | При большом количестве клиентов может быть неэффективно из-за вычисления SUM в оконной функции. |
...
| # | Подход | Плюсы | Минусы |
|---|--------|-------|--------|
| 1 | CTE с LAG для анализа соседних заказов | Использует оконные функции для сравнения последовательных заказов одного клиента. Чёткое условие на разницу дат и смену товаров. |  Работает, если в заказе только один товар, иначе потребуется агрегация. |
...
WITH
    ...
    SELECT
        ...
        LAG(oi.product_id) OVER w AS prev_product_id
    FROM
        orders o
    JOIN
        order_items oi
...
GROUP BY
    customer_id
WINDOW
    w AS (PARTITION BY o.customer_id ORDER BY o.order_date, o.order_id)
ORDER BY
    customer_id;
```
...
- **Многоступенчатые CTE** обеспечивают максимальную прозрачность и удобство отладки, но могут быть избыточными и менее производительными. Они полезны для сложных расчётов, где важна пошаговая проверка.
- **Оконные функции** позволяют сильно сократить код и часто выполняются эффективнее, однако требуют хорошего понимания порядка вычислений.
